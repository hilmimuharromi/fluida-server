...
const addPenProyek = async (req, res) => {
    try { 
      ...
      const isFound = await PenProyek.findOne({ user: payload.user, proyek: payload.proyek });
      if (isFound) {
...
const getPenProyek = async (req, res) => {
      try {
        ...
        if (user.role === "guru") {
          ...
          const result = await PenProyek.find({ user: req.decoded.id, proyek: req.params.proyekId }).populate('proyek').populate("user").sort({
            createdAt: "desc"
          });
          if (result) {
            const formatResult = result.map((item) => {
